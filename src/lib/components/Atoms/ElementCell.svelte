...
<div class="Elemento{legend ? " Legenda" : ""} {element.chemical_serie.replaceAll(" ", "-")} {element.state ?? "Desconhecido"}">
  <span class="atomic-number">{element.atomic_number}</span>
  <abbr class="symbol">{element.symbol}</abbr>
  <span class="name">{element.name}</span>
  <span class="atomic-weight">{element.short_lived ? "(" : ""}{element.atomic_weight}{element.short_lived ? ")" : ""}</span>
</div>
