...
for (const [macro, output] of Object.entries(macros)) {
	katex.__defineMacro(macro, output);
}
...
export function parseReaction(reaction) {
	const tokens = mhchemParser.go(reaction, "ce");
	...
	let side = "reactants";
	let buffer = []; // agrupa chemfive consecutivos

	const flushBuffer = (target) => {
		if (buffer.length === 0) {
			return;
		}
	...
	for (const token of tokens) {
		if (token.type_ === "arrow") {
			flushBuffer(side === "reactants" ? reactants : products);
			side = "products";

			// catalisadores acima da seta
			if (token.rd?.length) {
				for (const chem of token.rd) {
					flushBufferToArray([chem], catalysts);
				}
			}
...
// função auxiliar para catalisadores (como token.rd vem isolado)
function flushBufferToArray(tokensArray, target) {
	let formula = "";
	for (const node of tokensArray) {
		if (node.type_ !== "chemfive") {
			continue;
		}
		for (const part of node.o) {
			formula += part.p1;
...
const chemParse = (tokens, stateMachine) => {
	//console.log(tokens);
	// Recreate the argument string from KaTeX's array of tokens.
	let str = "";
	let expectedLoc = tokens.length && tokens.at(-1).loc.start;
...
const mhchemParser = {
	...
	patterns: {
		...
		findObserveGroups(
			...
			const _findObserveGroups = (input, i, endChars) => {
				let braces = 0;
	...
	actions: {
		...
		"insert+p1"(buffer, m, a) {
			return { type_: a, p1: m };
		},
		"insert+p1+p2"(buffer, m, a) {
		...
		"tex-math tight"(buffer, m) {
			return mhchemParser.go(m, "tex-math tight");
		},
		bond(buffer, m, k) {
			return { type_: "bond", kind_: k || m };
		},
		"color0-output"(buffer, m) {
...
mhchemParser.stateMachines = {
	...
	ce: {
		...
		actions: {
			...
			"beginsWithBond=true"(buffer) {
				buffer.beginsWithBond = true;
			},
			"beginsWithBond=false"(buffer) {
				buffer.beginsWithBond = false;
			},
			"parenthesisLevel++"(buffer) {
				buffer.parenthesisLevel++;
			},
			"parenthesisLevel--"(buffer) {
				buffer.parenthesisLevel--;
			},
			"state of aggregation"(buffer, m) {
			...
			output(buffer, m, entityFollows) {
				...
				if (buffer.r) {
					...
					if (
						...
						if (
							!(buffer.o || buffer.q || buffer.d || buffer.b || buffer.p) &&
							entityFollows !== 2
						) {
							buffer.o = buffer.a;
							buffer.a = undefined;
			...
			"oxidation-output"(buffer, m) {
				const ret = ["{"];
				mhchemParser.concatArray(ret, mhchemParser.go(m, "oxidation"));
				ret.push("}");
				return ret;
			},
			"frac-output"(buffer, m) {
				return {
					type_: "frac-ce",
